...
import { useCreateEvent, useCreateReminder, useGetDashboard } from '@workspace/api-client-react';
...
export default function AddUpdateScreen() {
  ...
  const params = useLocalSearchParams<{ profileId?: string; prefillTitle?: string; prefillType?: string }>();
  ...
  const save = () => {
    if (!selectedProfileId || !title.trim()) {
     ...
     void (async () => {
       try {
         await create.mutateAsync({ profileId: selectedProfileId, data: { type, title: title.trim(), description: `${description.trim()}${description.trim() ? ` · ${duration === 'short' ? 'Short-term' : 'Long-term'} attention` : `${duration === 'short' ? 'Short-term' : 'Long-term'} attention`}`, date: todayIso(), provider: provider.trim() || null, medications: medications.split(',').map((item) => item.trim()).filter(Boolean), followUp: followUp.trim() || null, tags: [duration] } });
         if (followUp.trim()) {
    ...
    <Screen scroll={false} contentStyle={{ paddingTop: 12 }}>
      <KeyboardAwareScrollViewCompat contentContainerStyle={styles.content} bottomOffset={80} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.field}><Text style={[styles.label, { color: colors.inkSoft }]}>A little more</Text><TextInput value={description} onChangeText={setDescription} placeholder="What would you like to remember?" placeholderTextColor={colors.mutedForeground} multiline style={[styles.input, styles.multiline, { color: colors.foreground, borderColor: colors.border, backgroundColor: colors.card }]} /></View>
         <Field label="Doctor or provider (optional)" value={provider} onChangeText={setProvider} placeholder="e.g. Dr. Sharma" colors={colors} />
         <Field label="Medication names (optional)" value={medications} onChangeText={setMedications} placeholder="Separate medicines with commas" colors={colors} />
         <Field label="Follow-up date (optional)" value={followUp} onChangeText={setFollowUp} placeholder="YYYY-MM-DD" colors={colors} />
