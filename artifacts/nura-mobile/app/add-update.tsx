    ...
    <Screen scroll={false} contentStyle={{ paddingTop: 12 }}>
      <KeyboardAwareScrollViewCompat contentContainerStyle={styles.content} bottomOffset={80} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.field}><Text style={[styles.label, { color: colors.inkSoft }]}>A little more</Text><TextInput value={description} onChangeText={setDescription} placeholder="What would you like to remember?" placeholderTextColor={colors.mutedForeground} multiline style={[styles.input, styles.multiline, { color: colors.foreground, borderColor: colors.border, backgroundColor: colors.card }]} /></View>
         ...
         <Field label="Test or lab result (optional)" value={testResult} onChangeText={setTestResult} placeholder="e.g. TSH — within range" colors={colors} />
         <View style={styles.field}><Text style={[styles.label, { color: colors.inkSoft }]}>Report (optional)</Text><OutlineButton label={reportName ? 'Replace attached report' : 'Attach report photo'} icon="paperclip" onPress={() => { void attachReport(); }} />{reportName ? <View style={styles.reportName}><Feather name="file-text" size={15} color={colors.primary} /><Text style={[styles.reportText, { color: colors.inkSoft }]} numberOfLines={2}>{reportName}</Text></View> : null}</View>
