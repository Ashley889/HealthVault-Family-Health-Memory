...
export default function RemindersScreen() {
  ...
  const completed = reminders.data.filter((item) => item.completed);
  const markComplete = (reminderId: number, title: string, profileId: number) => {
    complete.mutate({ reminderId }, {
      onSuccess: async () => {
        await queryClient.invalidateQueries();
        Alert.alert('Appointment completed', 'Would you like to add what happened to the health timeline?', [
          { text: 'Not now', style: 'cancel' },
          { text: 'Add visit details', onPress: () => router.push({ pathname: '/add-update', params: { profileId: String(profileId), prefillTitle: title, prefillType: 'visit' } }) },
        ]);
      },
      onError: () => Alert.alert('Couldn’t update reminder', 'Please try again.'),
    });
  };
  return (
    <Screen>
        ...
        return (
          <Card key={reminder.id} style={styles.reminderCard}>
            ...
            <View style={styles.reminderCopy}><Text style={[styles.reminderTitle, { color: colors.foreground }]}>{reminder.title}</Text><Text style={[styles.reminderDetail, { color: colors.mutedForeground }]}>{profile?.name || 'Family member'} · {reminder.detail}</Text><Text style={[styles.reminderDate, { color: colors.primary }]}>{formatDate(reminder.date)}</Text></View>
             <View style={[styles.statusPill, { backgroundColor: colors.accent }]}><Text style={[styles.statusText, { color: colors.primary }]}>Upcoming</Text></View>
             <Pressable accessibilityLabel={`Complete ${reminder.title}`} disabled={complete.isPending} onPress={() => markComplete(reminder.id, reminder.title, reminder.profileId)} style={[styles.complete, { borderColor: colors.border }]}><Feather name="check" size={17} color={colors.primary} /></Pressable>
          </Card>
        );
      })}
      {!open.length ? <Card><Text style={[styles.reminderTitle, { color: colors.foreground }]}>You’re all caught up.</Text><Text style={[styles.reminderDetail, { color: colors.mutedForeground }]}>Completed reminders stay out of the way.</Text></Card> : null}
       ...
       {completed.map((reminder) => {
         const profile = profiles.data.find((item) => item.id === reminder.profileId);
         return <Card key={reminder.id} style={[styles.reminderCard, styles.completedCard]}><View style={[styles.reminderIcon, { backgroundColor: colors.muted }]}><Feather name="check-circle" size={18} color={colors.success} /></View><View style={styles.reminderCopy}><Text style={[styles.reminderTitle, { color: colors.foreground }]}>{reminder.title}</Text><Text style={[styles.reminderDetail, { color: colors.mutedForeground }]}>{profile?.name || 'Family member'} · {reminder.detail}</Text><Text style={[styles.reminderDate, { color: colors.success }]}>{formatDate(reminder.date)}</Text></View><View style={[styles.statusPill, { backgroundColor: colors.muted }]}><Text style={[styles.statusText, { color: colors.success }]}>Completed</Text></View></Card>;
       })}
       {!completed.length ? <Card><Text style={[styles.reminderDetail, { color: colors.mutedForeground }]}>Completed visits will stay here and can be connected to the health timeline.</Text></Card> : null}
    </Screen>
  );
}
...
const styles = StyleSheet.create({
  ...
  statusPill: { borderRadius: 12, paddingHorizontal: 8, paddingVertical: 5 },
